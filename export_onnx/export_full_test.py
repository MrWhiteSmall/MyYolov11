...
class YOLO11:
    ...
    def postprocess(self, input_image, output):
        ...
        for i in range(rows):
            ...
            if max_score >= self.confidence_thres:
                class_id = np.argmax(classes_scores) # 获取最大置信度的下标
    ...
    def main(self):
        ...
        print('onnx总用时',time.time()-start_onnx_time)

 
        # 对输出进行后处理以获取输出图像
        output_image = self.postprocess(self.img, outputs)  # 输出图像
