...
class Exporter:
    ...
    def __call__(self, model=None,extra_metadata={},ir_version=None,) -> str:
        ...
        if engine:  # TensorRT required before ONNX
            f[1], _ = self.export_engine(ir_version=ir_version,)
        if onnx:  # ONNX
            f[2], _ = self.export_onnx(ir_version=ir_version,)
        if xml:  # OpenVINO
        ...
        if is_tf_format:  # TensorFlow formats
            ...
            f[5], keras_model = self.export_saved_model(ir_version=ir_version)
    ...
    @try_export
    def export_onnx(self, prefix=colorstr("ONNX:"),ir_version=None):
        """YOLOv8 ONNX export."""
        ...
        if ir_version is not None:
            model_onnx.ir_version = ir_version
    ...
    @try_export
    def export_engine(self, prefix=colorstr("TensorRT:"),ir_version=None,):
        """YOLOv8 TensorRT export https://developer.nvidia.com/tensorrt."""
        assert self.im.device.type != "cpu", "export running on CPU but must be on GPU, i.e. use 'device=0'"
        f_onnx, _ = self.export_onnx(ir_version=ir_version)  # run before TRT import https://github.com/ultralytics/ultralytics/issues/7016
    ...
    @try_export
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:"),ir_version=None):
        ...
        f_onnx, _ = self.export_onnx(ir_version=ir_version)
